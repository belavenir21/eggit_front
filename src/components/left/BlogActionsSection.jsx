        ...
        <div className="space-y-2">
            ...
            {isMe && (
                <div className="grid grid-cols-2 gap-2 mb-1.5">
                    <ActionButton icon={PlusSquare} label="생성" onClick={() => onNavigate('/blog/create')} className="blog-create-button" />
                    <ActionButton icon={PenTool} label="글쓰기" primary onClick={() => onNavigate('/blog/post')} className="blog-post-button" />
                </div>
            )}
            ...
            <div className="input-panel-icon">
                <button
                    onClick={onVisitBlog}
                    disabled={!hasBlog}
                    className={`w-full py-3 rounded-lg border-2 text-sm font-bold transition-all flex items-center justify-center gap-2
                    ${hasBlog
